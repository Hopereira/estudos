...
def iniciar_banco():
    ...
    # Adicionamos UNIQUE na coluna nome para o banco não aceitar duplicatas
    cursor.execute('''
...
def menu():
    while True:
        ...
        if opcao == "1":
            ...
            estilo = input("Estilo de Dança: ").strip()
            
            try:
                # O banco tentará inserir, se o nome já existir, ele lançará um IntegrityError
                cursor.execute('INSERT INTO alunos (nome, estilo) VALUES (?, ?)', (nome, estilo))
                conn.commit()
                print(f"✅ {nome} adicionado com sucesso!")
            except sqlite3.IntegrityError:
                print(f"⚠️ Erro: O aluno '{nome}' já está cadastrado no sistema.")
